 ...
 * it is throught X11, telnet, etc. We do this by printing the string to the
 * tty associated with current task.
 */
...
static void print_string(char *str)
{
	 ...
	 * if it is a deamon). If so, there is nothing we can do.
	 */
	...
	if (my_tty)
	{
		 ...
		 * kernel's memery segment.
		 *
		 * The functions 1st parameter is the tty to write to, because the
		 * same function would normally be used for all tty's of a certain
		 * type.
		 * The 2nd parameter is a pointer to a string.
		 * The 3rd parameter is the legnth of the string.
		 *
		 * As you will see below, sometimes it is necessary to use
		 * preprocessor stuff to create code that works for different
		 * kernel versions. The (naive) approach we have taken here does not
		 ...
		 *
		 * This is why test files are different between Unix and MS Windows.
