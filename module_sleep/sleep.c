/*
 * sleep.c - create a /proc file, and if sevceral processes try to open it
 * at the same time, put all buyt one to sleep.
 */
...
/* Here we keep the last message recieved, to prove that we can process our
 * input.
 */
 ...
 * output provisions - we have to use a standard read fuction, which is this
 * function.
 */
...
static int module_open(struct inode *inode, struct file *file)
{
	...
	/* This is the corrent place for try_module_get(THIS_MODULE) because if
	...
	while (atomic_cmpxchg(&already_open, 0, 1))
	{
		 ...
		 * is closed) or when a signal, sich as Ctrl-C, is sent to
		 * the process.
		 */
		wait_event_interruptible(waitq, !atomic_read(&already_open));

		/* If we woke up because we got a signal we are not blocking,
		...
		if (is_sig)
		{
			 ...
			 * an importal module, which can only be killed by rebooting
			 * the machine.
			 */
...
static int module_close(struct inode *inode, struct file *file)
{
	 ...
	 * be able to already_open back to one and to open the file. All the
	 * other processes will be called when already_open is back to one,
	 * so they will go back to sleep.
	 */
 ...
 * open function, which will get unloaded. I will explain how to avoid removal
